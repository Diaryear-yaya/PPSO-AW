...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_blur=True, do_rotate=True, rand_loc=True,
        ...
        # ----------------- 新增：补丁贴在目标下方 -----------------
        # 如果rand_loc为False，强制将补丁y坐标下移一定比例（如目标高的0.6倍）
        if not rand_loc:
            # lab_batch: [batch, num_obj, 5], 其中[..., 4]是h（归一化）
            target_h = lab_batch[:, :, 4].view(np.prod(batch_size))
            target_y = target_y + target_h * 0.6  # 下移0.6倍目标高
            # 防止越界
            target_y = torch.clamp(target_y, 0.0, 1.0)
        # ------------------------------------------------------
        ############################################################################################
...
class PatchApplier(nn.Module):
    ...
    def forward(self, img_batch, adv_batch, lab_idx):
        # Initialize an empty tensor with the same shape as img_batch
        new_img_batch = img_batch.clone()

        for i in range(len(img_batch)):
            advs = adv_batch[lab_idx == i]
            for adv in advs:
                # Replace the corresponding location in the new tensor (no in-place modification)
                new_img_batch[i] = torch.where((adv == 0), new_img_batch[i], adv)
...
class InriaDataset(Dataset):
    ...
    Attributes:
        len: An integer number of elements in the INRIA dataset.
    ...
    def __init__(self, img_dir, lab_dir, imgsize, cls_ids=0, pad=False,shuffle=True,use_heatmap=False, heatmap_cache_dir=None):
        if isinstance(cls_ids, int):
            cls_ids = [cls_ids]

        # 计算出数据集中所有png,jpg,bmp图片的数量。
        # fnmatch.filter(os.listdir(img_dir), '*.png')：使用 fnmatch 模块过滤出文件名匹配模式 *.png 的文件。*.png 表示所有以 .png 为后缀的文件。
        ...
        for img_name in self.img_names:
            self.img_paths.append(os.path.join(self.img_dir, img_name)) # 拼接图片路径并加入到列表中。
        if lab_dir is None:
            return
        img_names = []
        label_cache = {}
        n_labels = len(fnmatch.filter(os.listdir(lab_dir), '*.txt')) # 标签数量。
        ...
        for img_name in self.img_names:
            ...
            try:
                ...
                print(f"Warning: 标签文件 {lab_path} 不存在，跳过对应图像 {img_name}")
                continue  # 跳过这个图像，继续处理下一个
            if cls_ids is None:
                self.lab_paths.append(lab_path)
                img_names.append(img_name)
            else:
                clss = labels[:, 0] # 提取第一列所有数据。
                idxes = []
    ...
    def __getitem__(self, idx): # 预处理数据集。
        assert idx <= len(self), 'index range error'
        img_path = os.path.join(self.img_dir, self.img_names[idx])
        image = Image.open(img_path).convert('RGB')
        if self.lab_dir is None:
            image, _ = self.pad_and_scale(image)
            return to_tensor(image), None, None, (img_path, None) # to_tensor(image)将图片转化为张量。
    ...
    def pad_and_scale(self, img, lab=None): # 规范图片大小
        resize = transforms.Resize((self.imgsize, self.imgsize)) # 固定图片大小。
        if not self.pad:
        ...
        if w != h:
            ...
            if dim_to_pad == 1:
                ...
                padded_img.paste(img, (int(padding), 0)) # 是 Python 中 Pillow（PIL）库中的一个函数，\
                # 用于将一个图像粘贴到另一个图像上，或者在图像的某个特定区域中填充指定的颜色或图像
                img = padded_img
                if lab is not None:
...
def collate_fn(batch):
    ...
    # 批处理图像、标签、路径和形状，并为合并标签张量中的目标分配唯一的索引。
    b = zip(*batch) # zip(*batch) 会把 batch 中的每个元素解压，然后 zip 将每个位置上的元素组合在一起（矩阵转置）。
    im, label, label_idx, path = zip(*batch)  # transposed
    path = np.array(path).transpose() # path（通常是一个列表或列表的列表等可迭代对象）转换为一个 NumPy 数组，便于进行数值计算和数组操作。
    for i, lb in enumerate(label):
        if lb is None:
            # torch.stack() 用于将一系列相同形状的张量在新维度上堆叠。
            # dim=0 沿着第 0 维度将三个张量堆叠在一起，结果是一个新的张量。
            return torch.stack(im, 0), None, None, path
        label_idx[i][:] = i  # add target image index for build_targets()
    # torch.cat() 可以在指定维度上拼接张量，确保在拼接维度以外的维度上形状相同。
    return torch.stack(im, 0), torch.cat(label, 0), torch.cat(label_idx, 0), path

if __name__ == '__main__':
    from torch.utils.data import DataLoader

    torch.manual_seed(0) # 设置随机数生成器的种子函数
